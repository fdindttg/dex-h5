<template>
    <view>
        <view class="icon fn-center m-t-md">
            <img v-if="theme=='light'" src="static/img/light/Turn.png" @click="$emit('close')" class="w-30" alt="">
            <img v-else src="static/img/Turn.png" @click="$emit('close')" class="w-30" alt="">
        </view>
        ...
        <view class="d-flex justify-between align-center bg-panel-4 p-x-md border-t border-b p-y-xs">
            ...
            <view>
                <van-count-down class="color-default" :benginTime="current_scene.begin_time" :time="current_scene.lottery_time*1000" />
            </view>
        </view>
        <view class="form-item  m-y-md">
            ...
            <view class="box  income-box p-l-md">
                <view class="item  fn-center d-inline-block rounded-xs bg-panel-4 w-80 m-r-md" :class="{active:rangeUuid==item.uuid}" v-for="item in (typeObj.rangeList||[])" :key="item.uuid" @click="rangeUuid=item.uuid">
                    ...
                    <view class="income fn-10 color-light bg-panel-1">{{$t('option.b2')}}{{(item.odds*100).toFixed(0)}}%</view>
        ...
        <view class="form-item p-x-md m-t-md">
            ...
            <view class="d-flex p-x-xs rounded-xs">
                ...
                <view class="border-b flex-fill">
                    <v-input :placeholder="$t('common.至少买入') + omitTo(current_scene.min_money, 0)" v-model="num" type="number" class="color-light form-input w-max p-y-xs d-flex" />
                </view>
            </view>
        </view>
